...
class Texture:
    ...
    def __init__(self, file, isimg=False):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, self.image.width,
                     self.image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, self.img_data)
...
class Metal(Material):
    def __init__(self, name='_', color=glm.vec4(1.0, 1.0, 1.0, 1.0), sharpness=1.0, highlight_color=glm.vec4(1.0, 1.0, 1.0, 1.0), intensity=1.0, fresnel=1.0, mtype=1):
...
class Glass(Material):
    def __init__(self, name='_', color=glm.vec4(1.0, 1.0, 1.0, 1.0), IR=1.0,
                 highlight_color=glm.vec4(1.0, 1.0, 1.0, 1.0), intensity=1.0,
                 fresnel=1.0, sharpness=1.0, transparency=1.0):
        super().__init__(name, color, highlight_color)
        self.IR = IR
        self.sharpness = sharpness
        self.intensity = intensity
